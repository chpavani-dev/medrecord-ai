...
v1.6.0 — Date intelligence:
  - NEW: prescription_date extraction in /extract-drugs and /ocr/prescription
  - Tightened Claude prompts to aggressively find dates in Indian formats
  - Date normalization to YYYY-MM-DD (handles DD/MM/YYYY, DD-MM-YYYY, "April 28 2026", etc.)
...
def normalize_test_name(raw_name: str) -> str:
    """Normalize a test name to canonical form. Falls back to TitleCase."""
    if not raw_name:
        return raw_name
...
# ------------------------------------------------------------------
# Date normalization (NEW in v1.6.0)
# ------------------------------------------------------------------
def normalize_date(raw_date) -> Optional[str]:
    """
    Convert any date string Claude returns into YYYY-MM-DD format.
    Handles common Indian formats: DD/MM/YYYY, DD-MM-YYYY, DD.MM.YYYY,
    "April 28, 2026", "28 Apr 2026", "28/04/2026", and ISO already.
    ...
    # Try common formats — DD-first formats are PREFERRED for Indian docs
    patterns = [
        "%d/%m/%Y", "%d-%m-%Y", "%d.%m.%Y",  # DD/MM/YYYY (Indian standard)
        "%d/%m/%y", "%d-%m-%y",
        "%d %B %Y", "%d %b %Y",                # 28 April 2026, 28 Apr 2026
        "%B %d, %Y", "%b %d, %Y",              # April 28, 2026
        "%Y/%m/%d", "%Y-%m-%d",
        "%m/%d/%Y", "%m-%d-%Y",                # US format (last resort)
    ]
    for p in patterns:
        try:
            dt = datetime.strptime(s, p)
            # Sanity check: reject dates more than 1 year in the future or before 1950
            now = datetime.now()
...
class DrugExtractionResponse(BaseModel):
    ...
    engine: str
    drugs: list
    prescription_date: Optional[str] = None  # NEW in v1.6.0
    doctor_name: Optional[str] = None        # NEW in v1.6.0
    hospital_name: Optional[str] = None      # NEW in v1.6.0
...
# ==================================================================
# DRUG EXTRACTION (v1.6.0 — now extracts metadata too)
# ==================================================================
...
def extract_drugs_with_claude(ocr_text: str) -> dict:
    """
    Returns dict with: drugs (list), prescription_date, doctor_name, hospital_name.
    """
...
# ==================================================================
# LAB REPORT EXTRACTION (v1.5.0 + tighter date prompt)
# ==================================================================
...
def determine_flag(test_name: str, value, normal_range: str) -> Optional[str]:
    """Determine flag using standard ranges first, then report's range string."""
...
def parse_lab_report_with_claude(ocr_text: str) -> dict:
    ...
    try:
        ...
        # Normalize date BEFORE other processing
